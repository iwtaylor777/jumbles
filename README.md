...
A daily word game. Unscramble four common words, then use their **circled**
letters to crack a punny **bonus** answer. One new puzzle every day.

Live: **https://jumbles.vercel.app**
...
data/pool.json       curated 5-letter word pool (build-time output)
...
# 2. build the curated word pool
.venv/bin/python tools/build_pool.py 3.6
...
and finds four common words whose circled letters spell it. `tools/generate_batch.py`
holds the authored bonus/clue seeds and validates every puzzle before writing
`data/puzzles.json`.

To extend the calendar: add `(bonus, clue)` seeds to `tools/generate_batch.py`
and re-run step 3. (Automated nightly generation via GitHub Actions is planned.)
